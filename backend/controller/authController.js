...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    if (!questionExists) {
      logger.info(`registerUser: Security question not found: ${security_question_id}`);
    ...
    logger.info(`registerUser: Successfully registered user with ID: ${result.id}`);
    res.status(201).json({ 
      status: 'success', 
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    // セキュリティ回答の検証
    const isValid = await bcrypt.compare(security_answer, securityAnswer.answer_hash);
...
const getSecurityQuestion = async (req, res) => {
  ...
  try {
    ...
    logger.info(`getSecurityQuestion: Successfully fetched ${questions.length} questions`);
    res.status(200).json({ 
      status: 'success', 
      data: questions 
    });
